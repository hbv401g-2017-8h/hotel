...
public class BookingManager
{
	...
	public ArrayList<BookingNight> book(Guest guest, Room room, Payment payment,
			Date dateFrom, Date dateTo) throws DoubleBookedException
	{
		LocalDate start = dateFrom.toInstant().atZone(ZoneId.systemDefault())
				.toLocalDate();
		LocalDate end = dateTo.toInstant().atZone(ZoneId.systemDefault())
				.toLocalDate();
		...
		for (LocalDate date = start; date
				.isBefore(end); date = date.plusDays(1))
		{
